...
ggsave("CMRYGNY_expression.png",  h = 2.7, w = 4)
...
CDR3aa <- bcr.heavy %>% 
  ...
  dplyr::summarize(c_CDR3 = sum(count), f_CDR3 = sum(count)/seq_n ) %>%
  ungroup %>% distinct %>% arrange(-f_CDR3) %>% ungroup
CDR3_oxLDL <- CDR3aa %>% filter(CDR3aa == "CMRYSNYWYFDVW", cell_type != "B2", C == "IGHM")
###limit to IgM 
CDR3_oxLDL %>% ggplot(aes(x = condition, y = f_CDR3, fill = condition)) +
  geom_bar(stat = "identity", position = "dodge",  color = "black", linewidth = 1) +
  scale_fill_manual(values = vivid_colors) +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.9, hjust=1), 
        ...
        strip.text = element_text(
          size = 10)) + 
  facet_wrap(~cell_type) +
  ylab("% of all sequences") + xlab("") + ggtitle("CDR3AA: CMRYSNYWYFDVW")
ggsave("CMRYSNY_expression.png", h = 2.7, w = 4)
