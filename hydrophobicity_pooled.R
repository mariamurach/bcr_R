...
bcr_b1a  %>% filter(cell_type != "B2") %>%
  ggplot(aes(x = condition, y = hydrophobicity, fill = condition)) + 
  geom_boxplot() + theme_prism() + 
  scale_fill_manual(values = colo) + ggtitle("B-1a") + 
  ylab("hydrophobicity")+ stat_compare_means()
ggsave("plots/boxplot_hydrophobicity_b-1a.png", w = 5, h = 2.7)



bcr_b1a <- pool_CDR3_aa(bcr, "B-1b")
bcr_b1a$hydrophobicity <- hydrophobicity(bcr_b1a$CDR3aa)
...
bcr_b1a  %>% filter(cell_type != "B2") %>%
  ...
  ylab("hydrophobicity")+ stat_compare_means()
ggsave("plots/boxplot_hydrophobicity_b-1b.png", w = 5, h = 2.7)
