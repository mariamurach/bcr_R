...
bcr %>% filter(cell_type == "B-1a") %>% ggplot(aes(x = cdr3_length, fill = condition)) +
  ...
  theme_prism() + facet_wrap(~condition) + ggtitle("B-1a") + scale_fill_manual(values = colo)+ xlab("CDR3 length")
ggsave("plots/B1a_length.png", w = 6, h = 3.6)
bcr %>% filter(cell_type == "B-1b") %>% ggplot(aes(x = cdr3_length, fill = condition)) +
  geom_histogram(position = "dodge", bins = 50) +
  theme_prism() + facet_wrap(~condition) + ggtitle("B-1b")+ scale_fill_manual(values = colo) + xlab("CDR3 length")
ggsave("plots/B1b_length.png", w = 6, h = 3.6)
